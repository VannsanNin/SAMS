...
function StatCard({ item }) {
  ...
  return (
    <div className="impeccable-card p-5 flex flex-col justify-between group">
      ...
      <div className="mt-4">
        <p className="text-3xl font-display font-bold text-slate-900 dark:text-white tracking-tight">{item.value}{item.suffix ?? ''}</p>
  ...
  return (
    <div className="space-y-6 pb-12 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/80 dark:border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-display font-extrabold text-slate-900 dark:text-white tracking-tight">Teacher Console</h1>
        ...
        <div className="flex items-center gap-2.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200/80 dark:border-indigo-800 rounded-xl px-4 py-2.5 shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-6"><SubjectPerformance data={asArr(g.subject_performance)} /></div>
        <div className="lg:col-span-6"><AttendanceTrendChart data={asArr(g.attendance_trend)} /></div>
        <div className="lg:col-span-7"><GradeDistribution data={asArr(g.grade_distribution)} /></div>
        <div className="lg:col-span-5"><SubmissionRate data={asArr(g.submission_rate)} /></div>
                ...
                {l.today_timetable.map((s) => (
                  <li key={s.id} className="flex items-center justify-between border border-slate-100 dark:border-slate-800 rounded-xl p-3.5 bg-slate-50/50 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800 transition-all">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">{s.class || '—'}</p>
                    </div>
            ...
            {l.low_performing?.length ? (
              <ul className="space-y-2.5">
                {l.low_performing.map((s) => (
                  <li key={s.id} className="flex items-center justify-between border border-red-100 dark:border-red-900/40 bg-red-50/40 dark:bg-red-950/30 rounded-xl p-3.5">
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-red-700 dark:text-red-400 truncate">{s.name}</p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">{s.class || '—'}</p>
                    </div>
                    <span className="text-xs font-bold text-red-600 dark:text-red-400 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-md border border-red-200 dark:border-red-800">{s.marks}</span>
                  </li>
                ))}
              </ul>
            ...
            {l.upcoming_tests?.length ? (
              <ul className="space-y-2.5">
                {l.upcoming_tests.map((e) => (
                  <li key={e.id} className="border border-slate-100 dark:border-slate-800 rounded-xl p-3.5 bg-slate-50/50 dark:bg-slate-800/40">
                    <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">{e.name} · <span className="text-slate-500 dark:text-slate-400 font-normal">{e.subject}</span></p>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-1">{e.class || '—'} · <span className="font-bold text-amber-600 dark:text-amber-400">{e.date}</span></p>
                  </li>
                ))}
              </ul>
            ...
            {l.recent_submissions?.length ? (
              <ul className="space-y-2.5">
                {l.recent_submissions.map((s) => (
                  <li key={s.id} className="flex items-center justify-between border border-slate-100 dark:border-slate-800 rounded-xl p-3.5 bg-slate-50/50 dark:bg-slate-800/40">
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">{s.student || '—'} · <span className="text-slate-500 dark:text-slate-400 font-normal">{s.homework || '—'}</span></p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">{s.submitted_at || '—'}</p>
                    </div>
                    <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border ${s.status === 'graded' ? 'text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/60' : 'text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/60'}`}>
                      {s.status}
                    </span>
                  </li>
                ))}
        ...
        <div className="lg:col-span-5">
          <ListCard title="Messages & Notifications">
            {l.messages?.length ? (
              <ul className="space-y-2.5">
                {l.messages.map((m) => (
                  <li key={m.id} className={`border rounded-xl p-3.5 ${m.is_read ? 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900' : 'border-indigo-200 dark:border-indigo-800 bg-indigo-50/40 dark:bg-indigo-950/30'}`}>
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">{m.subject || 'Message'}</p>
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium pl-2 whitespace-nowrap">{m.date}</span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1 line-clamp-2">{m.body}</p>
                  </li>
                ))}
              </ul>
